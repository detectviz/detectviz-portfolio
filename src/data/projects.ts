...
export const projects: Project[] = [
  {
    ...
    content: '面對企業 IT 環境異質性高與數據孤島問題，獨立開發 iPOC 平台。挑戰在於需同時處理每日數億筆時序數據與跨平台（AIX/Windows/Linux）採集。採取 Agentless 架構降低部署門檻，運用 Golang 高併發特性優化數據清洗與寫入效能（如架構圖所示）。最終成功導入台積電、國泰金控等 10+ 客戶，建立 50+ 套標準化儀表板（含主介面與細部指標），提升交付效率 80% 並確保 99.9% 系統可用性。',
    display: true
  },
  {
    id: 'tsmc-pdu-monitor',
    ...
    content: '為滿足台積電資料中心高標準電力監控需求，需整合數百支不同廠牌 PDU 並統一 SNMP 與 Modbus 協定。設計從 Room 到 Branch 的 5 層式監控架構（參見架構圖），開發 17 張 Grafana 儀表板以視覺化關鍵指標（如 SPC Overview 與各層級視圖）。系統實現了即時負載追蹤與三相不平衡偵測，協助維運團隊主動發現潛在異常，大幅提升機房營運穩定性與管理效率。',
    display: true
  },
  {
    id: 'tcci-k8s-grafana-influxdb',
    title: '台泥儲能 Kubernetes 監控平台建置',
    category: '平台開發 (Software & Platform Engineering)',
    images: [
      'tcci-k8s-grafana-flowchart.png',
    ...
    description: '基於 Kubernetes 與 Redis 緩衝架構的高可用性監控平台',
    content: '針對台泥儲能對數據完整性的嚴格要求，在 Kubernetes 環境下建置高可用監控平台。挑戰在於防止網路不穩導致的數據遺失。設計以 Redis Stream 為核心的緩衝架構（如流程圖所示），並自主開發 Golang 工具 `redis2influx` 確保數據寫入的可靠性。整合 PVC 持久化存儲與 LDAP 認證，成功交付一套零資料遺失（參見即時儀表板）、具備彈性擴展能力的雲原生監控解決方案。',
    display: true
  },
  {
    ...
    content: '為驗證 InfluxDB 處理大規模工業物聯網 (IIoT) 數據的能力，主導科技廠房 PoC 專案。挑戰在於精準模擬數萬 Sensor 的併發寫入。自主開發支援多模式與重試機制的 Golang 壓力測試工具（如架構圖所示），取代制式方案。透過 Active/Backup 高可用架構與完整監控體系（如 Write Request 與 Error Count 圖表），成功量化系統極限與延遲表現，為客戶提供具數據支撐的容量規劃與架構優化建議。',
    display: true
  },
  {
    ...
    content: '面對醫院資訊系統 (HIS) 複雜的異質環境，需整合 AIX/Windows 等 5 種作業系統與 Oracle/IIS 服務。挑戰在於將 NMON, AWR 等 7 種日誌格式標準化。開發自動化 ETL 腳本清洗數據並寫入 InfluxDB（參見架構流程圖），建立 30+ 張跨層次關聯分析儀表板（如 IIS Daily/Monthly Overview）。最終成功識別磁碟耗盡、備份異常等 20+ 項關鍵隱患，實現從 OS 到 Application 的端到端可觀測性。',
    display: true
  },
  {
  ...
  },
  {
    ...
    title: '國泰金控 InfluxDB & Graylog 系統健檢',
    ...
    images: [
      ...
      'cathaybk-graylog-tuning-java-process.png'
    ],
    technologies: [
      { name: 'InfluxDB', icon: 'influxdb' },
      { name: 'Grafana', icon: 'grafana' },
    ...
    description: '金融級系統的 InfluxDB 容量規劃與 Graylog 異常排查',
    content: '針對國泰金控正式區執行 InfluxDB 與 Graylog 系統健檢。挑戰在於在不影響服務下識別效能瓶頸與異常根因。透過 7 天基準線監控，深入分析 InfluxDB High Cardinality Tag 影響與容量趨勢（如診斷圖表所示，包含 Bucket Size/Total 趨勢）。成功將 Graylog 資源異常追溯至特定 Java Process 與 Topic 分配變更，並提出具體的 Tag 優化與維運 SOP 建議，確保金融級核心系統的長期穩定。',
    display: true
  },
  {
    ...
    content: '為產險公司核心系統執行 AIX 與 Informix 資料庫健檢。挑戰在於從海量日誌中挖掘潛在風險。透過整合分析 NMON 與 onstat 數據（參見分析流程圖），精準識別出 I/O 瓶頸、磁碟空間耗盡風險、異常程序滯留（如 Process 與 Onstat 圖表）及備份排程衝突等 20+ 項關鍵問題。提供包含資源配置優化與排程調整在內的具體改善方案，有效預防系統中斷風險。',
    display: true
  },
  {
    ...
    content: '協助客戶驗證 Pure Storage 升級效益，挑戰在於解釋架構改變導致的硬碟 Busy% 上升現象。透過數據分析（參見架構圖與 System Metrics），證實雖然 Busy% 增加，但讀取吞吐量提升 375%、IOPS 提升 39%，成功消除客戶疑慮。進一步執行 dd 與 scp 壓力測試量化 I/O 與網路極限（如 Stress Test Charts），並據此提出負載分散與備份排程優化的專業建議，確保新系統發揮最大效能。',
    display: true
  },
  {
    ...
    content: '受邀擔任台大計算機中心講師，挑戰在 3 小時內帶領 71 位學員掌握時序資料庫應用。精心設計涵蓋 TSDB 原理、Telegraf 採集、Flux 查詢到 Grafana 儀表板實作的 8 大模組課程（參考課程大綱圖與教學現場）。透過循序漸進的教學與實機演練（如 Grafana Query 與 InfluxDB UI 操作），成功協助學員跨越技術門檻，建立數據視覺化與監控告警的實戰能力，達成技術推廣與知識傳承的目標。',
    links: [
  ...
  },
  {
    ...
    content: '鑑於人工執行台灣政府組態基準 (TWGCB) 耗時且易錯，開發開源自動化框架。採用模組化 Bash Shell 架構，將 200+ 條 RHEL 9 安全規範轉化為可執行的 Check/Apply 雙模式腳本（參見自動化流程圖）。實作完整的日誌稽核系統，不僅大幅縮短合規設定時間，更確保了操作的可追溯性與一致性，為系統管理者提供高效且可靠的資安合規工具。',
  ...
  },
  {
    ...
    content: '針對 API 版本管理與演進追蹤的需求，開發輕量級視覺化工具。挑戰在於不依賴複雜資料庫即可展示歷史軌跡。利用 Git blame 分析 Swagger 文件變更，結合 Golang 開發零依賴 HTTP Server 輸出 CSV 數據（如流程圖所示）。整合 Grafana Infinity Plugin，創新地實現了 API 新增趨勢與負責人的即時視覺化（參見 Dashboard），有效協助團隊識別技術債與規劃版本迭代。',
  ...
  },
  {
    ...
    content: '旨在展示企業級雲原生基礎設施的實踐能力，建構從 KVM 虛擬機 (Terraform) 到 Kubernetes (Ansible) 再到應用交付 (Argo CD) 的全流程自動化平台。挑戰在於整合複雜的工具鏈與安全機制。實作完整的 GitOps 流程（參見架構圖），深度整合 Vault 秘密管理、Keycloak OIDC 認證及 Prometheus/Loki/Tempo 可觀測性堆疊，打造現代化、安全且可觀察的基礎設施範本。',
